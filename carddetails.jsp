...
Connection con=DriverManager.getConnection("jdbc:odbc:netbank","root","root");
    ...
    rs=st.executeQuery("select max(credit_card_no) from auto");
    if(rs.next()) {
	crcardno=rs.getInt(1);
	crcardno+=1;
	pst=con.prepareStatement("insert into auto(credit_card_no) values(?)"); 
   ...
   %>
<table style="border:1px solid gray" align="center" id=register width=500 cellspacing=0 cellpadding=2><form action="CreditCardIssue.jsp" method="post" onsubmit="return validate(this)">
<tr><td align=right>CardNo</td><td><input type="text" name="cardnum" value="<%=top%>" maxlength=10 readonly /></td></tr>
